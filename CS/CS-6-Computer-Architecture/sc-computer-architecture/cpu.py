...
class CPU:
    ...
    def __init__(self):
        ...
        self.E = None # equal flag, used in CMP, initialize
        self.L = None # less than flag, used in CMP, initialize
        self.G = None # greater than flag, used in CMP, initialize
        ...
        self.instruction_map = {
            ...
            0b10100111: self.handle_CMP,  # CMP (compare)
            0b01010101: self.handle_JEQ,  # JEQ (jump if equal)
            0b01010110: self.handle_JNE,  # JNE (jump if not equal)
            0b01010100: self.handle_JMP   # JMP (jump)
            }
    ...
    def alu(self, op, reg_a, reg_b):
        ...
        if op == "ADD":
            ...
            self.L = (self.reg[reg_a] < self.reg[reg_b])
            self.G = (self.reg[reg_a] > self.reg[reg_b])
        elif op == 'AND':
            ...
            if reg_b == 0:
                print('Cannot modulo by zero. exiting')
                exit()
            self.reg[reg_a] = self.reg[reg_a] % self.reg[reg_b]
        else:
    ...
    def handle_CMP(self, reg1, reg2):
        """compares the values in two registers
        if they are equal set equal (E) flag to 1, otherwise, 0"""
        self.alu('CMP', reg1, reg2)

    def handle_JEQ(self, reg, _):
        """ if equal flag set to 1 (handled by CMP), 
        jump to specified register"""
        self.pc += 2 # default to raising by 2
        if self.E:
            self.pc = self.reg[reg] # change pc to value in specified reg

    def handle_JNE(self, reg, _):
        """ if equal flag set to 0 (handled by CMP), 
        jump to specified register"""
        self.pc += 2
        if not self.E:
            self.pc = self.reg[reg] # change pc to value in specified reg

    def handle_JMP(self, reg, _):
        """ Jump to specified register"""
        self.pc = self.reg[reg] # change pc to valie in specified reg 
